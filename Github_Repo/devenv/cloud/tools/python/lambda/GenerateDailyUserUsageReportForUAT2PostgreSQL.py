...
def process_data():
    ...
    file_attribute = File_Attribute(s3_bucketname, folder_cercreation(), 'tenants_system_prod_uat2')
    ...
    file_attribute = File_Attribute(s3_bucketname, folder_cercreation(), 'tenants_prod_uat2')
    generate_report(db_attrib, quries, file_attribute)
    ...
    file_attribute = File_Attribute(s3_bucketname, folder_cercreation(), 'objExecEMRCluster_prod_uat2')
    generate_report(db_attrib, quries, file_attribute)
    ...
    quries = Queries("public", "ds_1_null____executi_459779378", sql_column_filter, sql_content,column_results)
    file_attribute = File_Attribute(s3_bucketname, folder_cercreation(), 'objExecDedicatedCluster_prod_uat2')
    generate_report(db_attrib, quries, file_attribute)
    ...
    quries = Queries("public", "", sql_column_filter, sql_content, column_results)
    file_attribute = File_Attribute(s3_bucketname, folder_cercreation(), 'environmentName_uat2')
    generate_report(db_attrib, quries, file_attribute)
...
def lambda_handler(event, context):
    process_data()
    sendListsToSNS(PROD_TOPICARN, boto3.Session(), "Daily postgreSQL report", "Daily postgreSQL usage report(uat) in Sagacity account. ", "Daily postgreSQL usage report(uat) generated.")
