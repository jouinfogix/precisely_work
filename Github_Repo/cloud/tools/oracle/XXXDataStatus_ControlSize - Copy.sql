...
HM_COUNT NVARCHAR2(50);
HM1_COUNT NVARCHAR2(50);
MHS_COUNT NVARCHAR2(50);
...
WITH CTE_ALL AS(
SELECT
   X.TABLE_NAME AS TABLE_NAME, X.bytes/1024/1024 AS BytesInMB
FROM
...
WHERE EXISTS(SELECT T.table_name, T.tablespace_name FROM dba_tables T 
             WHERE T.table_name = X.table_name AND T.owner = X.owner
                   AND T.owner=''IAUSER'')),
CTE_HM AS (
 SELECT TRUNC(SUM(CTE_ALL.BytesInMB)) AS SizeMB 
...
SELECT HM.SizeMB,  HM1.SizeMB ,MHS.SizeMB, HMBR.SizeMB INTO 
      HM_COUNT, HM1_COUNT, MHS_COUNT, HMBR_COUNT
FROM CTE_HM HM, CTE_HM1 HM1, CTE_MHS MHS, CTE_HMBR HMBR;
      
v_file_handle:=utl_file.fopen(''BDUMP'',''alert_'' || DBNAME || ''.log'', ''a'');
IF(HM_COUNT IS NOT NULL) THEN
   OUTPUT := ''CUSTOMLOG;HM;'' || HM_COUNT || '';'' || DATETIMESTR;
   utl_file.put_line (v_file_handle, OUTPUT);
END IF;
IF(HM1_COUNT IS NOT NULL) THEN
   OUTPUT := ''CUSTOMLOG;HM1;'' || HM1_COUNT || '';'' || DATETIMESTR;
   utl_file.put_line (v_file_handle, OUTPUT);
END IF;
IF(MHS_COUNT IS NOT NULL) THEN
   OUTPUT := ''CUSTOMLOG;MHS;'' || MHS_COUNT || '';'' || DATETIMESTR;
   utl_file.put_line (v_file_handle, OUTPUT);
END IF;
