...
DECLARE
		...
		@UniqueIndexName VARCHAR(50) = 'B_MASTER_REP_UNIQ_ITEM_ID';
		...
		BEGIN
			...
			SET @UniqueIndexName = 'B_REP_HIST_UNIQ_ITEM_ID';
		END;
...
DECLARE
		...
		@PrimaryKeyFileGroupName VARCHAR(100),
...
BEGIN
	...
	SET @SQL_CreateFullTextIndex = 'CREATE FULLTEXT INDEX ON [dbo].' + QUOTENAME(@MainTableName) + '(' + QUOTENAME(@FullTextColumnName) + ')'
            + 'KEY INDEX ' + QUOTENAME(@UniqueIndexName) + ';' + @NewLine;
END;
...
-- Must create Unique index as Full-Text index, and it must be single column,

PRINT @NewLine + 'CREATE UNIQUE INDEX ' + QUOTENAME(@UniqueIndexName) + ' ON ' + QUOTENAME(@MainTableName) + '(' + QUOTENAME(@PrimaryKeyColumnName) + ') ON ' + QUOTENAME(@PrimaryKeyFileGroupName) + ';' + @NewLine + 'GO' + @NewLIne;
