...
def get_untagged_rds_instances(session, account, region):
    ...
    for rds in dbInstances:
        ...
        if tagList:
           found = False
           for tag in tagList:
               if tag['Key'] and tag['Value'] and tag['Key'] in ['CostCenterID','CostCategoryID'] and tag['Value'] in ['LIC', 'CLC', 'POC', 'SVC', 'DEV']:
                   found = True
                   break
...
def check_untagged_or_unattached_EBS_volumes(event, context):
    try:
        ...
        publish_session = boto3.Session(profile_name=PROFILE, region_name=topic_existed_on_region)
        ...
        for region in regions:
            ...
            if len(untagged_list) > 0:
                sendListsToSNS(topic, publish_session, "Untagged EBS Volumes",
                               "AWS Account: <{}> at region: {} has untagged resources".format(get_account_id(session),
                                                                                          region), untagged_list)

            if len(unattached_list) > 0:
                sendListsToSNS(topic, publish_session, "Unattached EBS Volumes",
                               "AWS Account: <{}> at region: {} has unattached resources".format(get_account_id(session),
                                                                                            region), unattached_list)

            untagged_rds_instances = get_untagged_rds_instances(session, get_account_id(session), region) # event['account'])
            if len(untagged_rds_instances) > 0:
                sendListsToSNS(topic, publish_session, "Untagged RDS instances",
                                               "AWS Account: <{}> at region: {} has untagged RDS instances".format(get_account_id(session),
                                                                                         region), untagged_rds_instances)
    except Exception as err:
        logger.error("Failed to get account ID with error {}!".format(err))
