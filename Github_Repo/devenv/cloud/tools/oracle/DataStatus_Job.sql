...
OUTPUT := '''';
FOR rec IN (
        SELECT DT.OWNER, ROUND((DT.data_size + IDX.index_size)/1024/1024/1024, 2) AS TOTALSIZE_GB
	FROM
	(select tbl.OWNER, sum(col.AVG_COL_LEN * tbl.NUM_ROWS) as data_size
	from dba_tables tbl, dba_tab_cols col
	where tbl.table_Name = col.table_name and tbl.OWNER = col.OWNER
	      and tbl.OWNER like ''%USER''
	GROUP BY tbl.OWNER) DT,
	(select tbl.owner, sum(col.AVG_COL_LEN * tbl.NUM_ROWS) as index_size
	from dba_ind_columns idx, dba_tables tbl, dba_tab_cols col
	where tbl.table_name = idx.table_name and idx.table_Name = col.table_name
	     and tbl.OWNER = col.owner and idx.INDEX_OWNER = tbl.OWNER
	     and idx.column_name = col.column_name and tbl.OWNER like ''%USER''
	GROUP BY tbl.OWNER) IDX
        WHERE DT.OWNER = IDX.OWNER
        )
   LOOP
      OUTPUT := OUTPUT || ''CUSTOMLOG;PRODUCTSIZE;'' || rec.OWNER || ''|'' || rec.TOTALSIZE_GB || '';'' || DATETIMESTR || CHR(13) || CHR(10);
   END LOOP;
...
CREATE TABLESPACE IA_DYNOBJECTS_IDX DATAFILE SIZE 100M AUTOEXTEND ON NEXT 100M EXTENT MANAGEMENT LOCAL AUTOALLOCATE SEGMENT SPACE MANAGEMENT AUTO; 
ALTER USER IAUSER  QUOTA UNLIMITED ON IA_DYNOBJECTS_IDX; 

CREATE TABLESPACE IA_DYNOBJECTS_IDX2 DATAFILE SIZE 100M AUTOEXTEND ON NEXT 100M EXTENT MANAGEMENT LOCAL AUTOALLOCATE SEGMENT SPACE MANAGEMENT AUTO; 
ALTER USER IAUSER  QUOTA UNLIMITED ON IA_DYNOBJECTS_IDX2; 
/
***/
...
CREATE TABLESPACE ER_DYNOBJECTS_IDX DATAFILE SIZE 100M AUTOEXTEND ON NEXT 100M EXTENT MANAGEMENT LOCAL AUTOALLOCATE SEGMENT SPACE MANAGEMENT AUTO; 
ALTER USER ERUSER  QUOTA UNLIMITED ON ER_DYNOBJECTS_IDX; 


CREATE TABLESPACE ER_DYNOBJECTS_IDX2 DATAFILE SIZE 100M AUTOEXTEND ON NEXT 100M EXTENT MANAGEMENT LOCAL AUTOALLOCATE SEGMENT SPACE MANAGEMENT AUTO; 
ALTER USER ERUSER  QUOTA UNLIMITED ON ER_DYNOBJECTS_IDX2; 
/
***/
